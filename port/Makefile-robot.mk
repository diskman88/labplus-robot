...
# Firmware verion 
MICROPY_GIT_TAG = $(strip $(shell git describe --dirty --always))
...
ESPIDF_SUPHASH_V3 := 722043f734fa556d66d57473ac266fb1d0ec5ad2
# ESPIDF_SUPHASH_V4 := 4c81978a3e2220674a432a588292a4c860eef27b
ESPIDF_SUPHASH_V4 := f56e8baef7fd663d590e6d3b03c91db0a632fb39
...
# 增加运行时的异常屏幕显示功能;CTRL+C中断停止所有线程和定时器
LIB_SRC_C += lib/utils/pyexec.c
...
# 开机显示和异常打印功能模块
INC += -Idrivers
INC += -Idrivers/startup
STARTUP_SRC_C = $(addprefix drivers/startup/, \
	oled.c \
	i2c_master.c \
	startup.c \
	)
# 音频功能模块
INC += -Idrivers/codec/codec_lib/helix/include
INC += -Idrivers/codec/decoder/include
INC += -Idrivers/codec/es8388
INC += -Idrivers/codec/include
CFLAGS += -DARM -DCONFIG_AUDIO_HELIX
CODEC_SRC_C = $(addprefix drivers/codec/,\
	audio_player.c \
	url_codec.c \
	local_play.c \
	http_client.c \
...
# mpu6050驱动
# mpu6050 driver
INC += -I drivers/mpu6050/driver/include
INC += -I drivers/mpu6050/driver/eMPL
INC += -I drivers/mpu6050/driver/esp32
INC += -I drivers/mpu6050/eMPL-hal
INC += -I drivers/mpu6050/mpl
INC += -I drivers/mpu6050/mllite
DRIVERS_SRC_C += $(addprefix drivers/,\
	mpu6050/driver/eMPL/inv_mpu_dmp_motion_driver.c \
	mpu6050/driver/eMPL/inv_mpu.c \
	mpu6050/driver/esp32/log_esp32.c \
	mpu6050/eMPL-hal/eMPL_outputs.c \
	mpu6050/mllite/data_builder.c \
	mpu6050/mllite/hal_outputs.c \
	mpu6050/mllite/message_layer.c \
	mpu6050/mllite/ml_math_func.c \
	mpu6050/mllite/mlmath.c \
	mpu6050/mllite/mpl.c \
	mpu6050/mllite/results_holder.c \
	mpu6050/mllite/start_manager.c \
	mpu6050/mllite/storage_manager.c \
	)	

# 5x5 RGB点阵图形库
# 5x5 RGB matrix graphic lib
INC += -Idrivers/rgbLED5x5
DRIVERS_SRC_C += $(addprefix drivers/,\
	rgbLED5x5/constimage.c \
	rgbLED5x5/image.c \
	rgbLED5x5/rgb_display.c \
	)	
...
# mpython builtin modules
INC += -Ibuiltins
BUILTIN_C ?= $(addprefix builtins/, \
	machine_pin.c \
	machine_servo.c \
	machine_touchpad.c \
	modmusic.c \
...
# mpython project builtin modules source files 
# board files, normal is main.c
BOARD_C ?= $(wildcard $(BOARD_DIR)/*.c)

# 过滤micropython esp32 port中的未修改的所有源文件
# filter out the source files that not modified in microPython esp32 port
INC += -I$(TOP)/ports/esp32
SRC_C := $(addprefix ports/esp32/, $(filter-out $(notdir $(BOARD_C)) $(notdir $(BUILTIN_C)), $(SRC_C)))

# mpython项目中的源文件
# the source files of mpython
PY_O := $(filter-out $(BUILD)/extmod/modframebuf.o, $(PY_O))
...
# 去除编译mpu6050官方库时一些警告信息
# add compile flags to disable some warnings.
$(BUILD)/drivers/mpu6050/%.o: CFLAGS += -Wno-unused-but-set-variable -Wno-unused-local-typedefs -Wno-double-promotion -Wno-float-conversion
...
######################################################################
# add esp componets for audio 
# nghttp
CFLAGS += -I$(ESPCOMP)/nghttp/port/include
CFLAGS += -I$(ESPCOMP)/nghttp/nghttp2/lib/includes
ESPIDF_NGHTTP_O = $(patsubst %.c,%.o, $(wildcard $(ESPCOMP)/nghttp/port/*.c)) 
$(eval $(call gen_espidf_lib_rule,nghttp,$(ESPIDF_NGHTTP_O)))
# esp_http_client
CFLAGS += -I$(ESPCOMP)/esp_http_client/include
CFLAGS += -I$(ESPCOMP)/esp_http_client/lib/include
ESPIDF_ESP_HTTP_CLIENT_O = $(patsubst %.c,%.o, \
...
# tcp_transport
CFLAGS += -I$(ESPCOMP)/tcp_transport/include
CFLAGS += -I$(ESPCOMP)/tcp_transport/private_include
...
# esp_tls
CFLAGS += -I$(ESPCOMP)/esp-tls
CFLAGS += -I$(ESPCOMP)/esp-tls/private_include
...
$(BUILD)/$(FIRMWARE_NAME).bin: $(BUILD)/bootloader.bin $(BUILD)/partitions.bin $(BUILD)/application.bin Noto_Sans_CJK_SC_Light16.bin
	$(ECHO) "Create $@, with address from 0"
	$(Q)$(PYTHON) release.py $^ $@

release: $(BUILD)/$(FIRMWARE_NAME).bin
	$(ECHO) "Writing $^ to the board"
	$(Q)$(ESPTOOL) --chip esp32 --port $(PORT) --baud $(BAUD) write_flash -z --flash_mode $(FLASH_MODE) --flash_freq $(FLASH_FREQ) 0x00 $^

font:
	$(ECHO) "Write $^ to the board"
	$(Q)$(ESPTOOL) --chip esp32 --port $(PORT) --baud $(BAUD) write_flash -z --flash_mode $(FLASH_MODE) --flash_freq $(FLASH_FREQ) 0x400000  Noto_Sans_CJK_SC_Light16.bin
...
APP_LD_ARGS += -L$(ESPCOMP)/esp_wifi/lib_esp32 -lcore -lmesh -lnet80211 -lphy -lrtc -lpp -lsmartconfig -lcoexist -lespnow
else
APP_LD_ARGS += $(ESPCOMP)/esp32/libhal.a
APP_LD_ARGS += -L$(ESPCOMP)/bt/lib -lbtdm_app
APP_LD_ARGS += -L$(ESPCOMP)/esp32/lib -lcore -lmesh -lnet80211 -lphy -lrtc -lpp -lwpa -lsmartconfig -lcoexist -lwps -lwpa2 -lespnow
endif
APP_LD_ARGS += $(OBJ)
APP_LD_ARGS += $(LIB)
...
$(BUILD)/partitions.bin: $(PART_SRC)
	$(ECHO) "Create $@"
	$(Q)$(PYTHON3) $(ESPCOMP)/partition_table/gen_esp32part.py -q $< $@
